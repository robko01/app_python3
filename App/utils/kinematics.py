...
class Kinematics:
    """Kinematics model of the Robko 01"""
...
#region Private Methods
    ...
    def inverse_kinematic(self, X0, Y0, Z0, P0, R0):
    ...
    def rights_kinematic(self, T1, T2, T3, T4, T5):
        """Right kinematics solver."""

        RP = 0.0

        PP = (T4 + T5) / 2
        RR = (T4 - T5) / 2 - self.__R1 * T1
        RP = self.__L1 * math.cos(T2)\
            + self.__L2 * math.cos(T3)\
            + self.__LG * math.cos(PP)

        XX = RP * math.cos(T1)
        YY = RP * math.sin(T1)
        ZZ = self.__H\
            - self.__L1 * math.sin(T2)\
            - self.__L2 * math.sin(T3)\
            - self.__LG * math.sin(PP)
